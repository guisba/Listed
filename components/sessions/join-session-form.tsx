...
export function JoinSessionForm({ initialCode = "" }: { initialCode?: string }) {
  ...
  return <form onSubmit={submit} className="space-y-5"><label className="block space-y-2"><span className="text-sm font-semibold">Código da sessão</span><Input value={code} onChange={(event) => setCode(event.target.value.toUpperCase().replace(/[^A-Z2-9]/g, "").slice(0, 6))} placeholder="7KQ9XM" className="h-14 text-center font-mono text-xl font-bold uppercase tracking-[.25em]" autoFocus /></label><label className="block space-y-2"><span className="text-sm font-semibold">Seu nome ou apelido</span><Input value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="Como a galera te chama?" maxLength={40} /></label>{error ? <div role="alert" className="flex gap-3 rounded-2xl border border-destructive/25 bg-destructive/10 p-4 text-sm"><ShieldAlert className="mt-0.5 size-4 shrink-0" />{error}</div> : null}<Button type="submit" size="lg" className="w-full" disabled={isLoading}>{isLoading ? <><LogIn className="size-4 animate-pulse" /> Entrando…</> : <>Entrar na sessão <ArrowRight className="size-4" /></>}</Button></form>;
}
