...
import type { AuthProviderAvailability } from "@/lib/supabase/auth-providers";

export function LoginForm({ providers }: { providers: AuthProviderAvailability }) {
  const { t } = useI18n();
  ...
  const hasOAuthProvider = providers.google || providers.discord;

  return <div className="space-y-5">{hasOAuthProvider ? <><div className="grid gap-3 sm:grid-cols-2">{providers.google ? <Button variant="secondary" onClick={() => oauth("google")}><Globe2 className="size-4" /> Google</Button> : null}{providers.discord ? <Button variant="secondary" onClick={() => oauth("discord")}><MessageCircle className="size-4" /> Discord</Button> : null}</div><div className="flex items-center gap-3 text-xs text-muted-foreground"><span className="h-px flex-1 bg-border" />{t("login.orEmail")}<span className="h-px flex-1 bg-border" /></div></> : null}<form onSubmit={emailLogin} className="space-y-3"><label className="block space-y-2"><span className="text-sm font-semibold">{t("login.email")}</span><div className="relative"><AtSign className="absolute left-4 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder={t("login.emailPlaceholder")} className="pl-11" required /></div></label><Button className="w-full" type="submit" disabled={loading}>{loading ? <LoaderCircle className="size-4 animate-spin" /> : <Mail className="size-4" />} {t("login.magicLink")}</Button></form>{status ? <p role="status" className="rounded-lg border border-border bg-secondary p-4 text-sm">{status}</p> : null}<p className="text-xs leading-5 text-muted-foreground">{t("login.preserve")}</p></div>;
}
